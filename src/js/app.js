...
(function () {
	...
	// Prevent menu collapse on mobile when clicking submenu toggles
	function preventMenuCollapseOnMobile() {
		/**
		 * Prevent default behavior on sidebar menu links with collapse toggle in mobile
		 */
		if (window.innerWidth <= 767) {
			// Add a global click handler to prevent menu collapse when clicking inside the sidebar
			document.addEventListener("click", function (e) {
				var sidebar = document.querySelector(".app-menu");
				if (sidebar) {
					var isInside = sidebar.contains(e.target);
					if (isInside) {
						// Click is inside the sidebar, prevent any menu collapse behavior
						e.stopPropagation();
					}
				}
	...
	function initLeftMenuCollapse() {
		...
		// Prevent menu collapse on mobile after menu reload
		preventMenuCollapseOnMobile();
	}

	function isLoadBodyElement() {
		var verticalOverlay = document.getElementsByClassName("vertical-overlay");
		if (verticalOverlay) {
			Array.from(verticalOverlay).forEach(function (element, index) {
				element.addEventListener("click", function (e) {
